...
UE.Editor.prototype.getActionUrl = function(action) {
    ...
    return this.getOpt('ossUploadUrl');
}

function ossService(editor, data) {
    ...
    $.ajax({
        url: editor.getOpt('ossPolicyUrl'),
        type: 'get',
        async: false,
        dataType: 'json',
...
window.UEDITOR_CONFIG['imageUploadService'] = function(context, editor) {
    return {
        ...
        setFormData: function(object, data, headers) {
            var ossData = ossService(editor, data);
            object.file.url = ossData.url;
            data.OSSAccessKeyId = ossData.OSSAccessKeyId;
            data.policy = ossData.policy;
            data.Signature = ossData.Signature;
            data.key = ossData.key;
            // data.file = ossServiceData.file;
            return data;
        },
        /**
         * 触发startUpload事件时执行
         ...
         * @param {Object} file 当前上传对象
         * @param {Object} res 上传接口返回的response
         * @returns {Boolean} 上传接口返回的response成功状态条件 (比如: res.code == 200)
         */
        getResponseSuccess: function(file, res) {
            var responseText = res._raw;
            if (responseText === '') {
                res.url = file.url;
                return true;
            }else {
...
window.UEDITOR_CONFIG['scrawlUploadService'] = function(context, editor) {
    return scrawlUploadService = {
        ...
        uploadScraw: function(file, base64, success, fail) {
            ...
            $.getJSON(editor.getOpt('ossPolicyUrl'), function (result) { //签名用的PHP
                if (!result.success) {
                    bootbox.alert(result.message);
                    return false;
                } else {
                    ...
                    formData.append('key', result.data.dir + ossFilename);
